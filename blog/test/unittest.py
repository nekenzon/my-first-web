...
class Myunittest(TestCase):
    ...
    def test_skill(self):
        temp = self.bs.select("table > tr.skill ")[0].select("td")[0]
        self.assertEqual("Skill-And-Hobbit", temp.text)
        temp = self.bs.select("table > tr.skill ")[0].select("td")[1]
        self.assertEqual("#Stay Up All night for programming #Test Open-Source Game or App #Develop Game", temp.text)

    def test_evaluation(self):
        temp = self.bs.select("table > tr.evaluation ")[0].select("td")[0]
        self.assertEqual("Self-Evaluation", temp.text)
        temp = self.bs.select("table > tr.evaluation ")[0].select("td")[1]
